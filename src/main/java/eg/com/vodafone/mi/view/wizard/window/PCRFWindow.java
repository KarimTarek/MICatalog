...
public class PCRFWindow extends AbstractWindow
{
	...
	cmboOperation.addValueChangeListener(new ValueChangeListener()
	{
		...
		{
		    txtSubject.setValue("VFE.SERVICES.unsubscribePCRFPolicy.REQ");
		}
	    }
	});
	...
	txtSrvPkg = new TextField("Service Package: ");
	txtSrvPkg.setSizeFull();
	txtSrvPkg.setValue(getMapValue("SERVICEPACKAGE"));
	formLayout.addComponent(txtSrvPkg);
	
	txtQuota = new TextField("Quota Name: ");
	txtQuota.setSizeFull();
	txtQuota.setValue(getMapValue("QUOTANAME"));
	formLayout.addComponent(txtQuota);
	...
	if (isNewConf())
	{
	    ...
	    this.stepBean.setType("PCRF");

	    Map<String, String> serviceConf = new HashMap<String, String>();
	    serviceConf.put("SERVICE", this.txtServiceName.getValue());
	    serviceConf.put("SERVICEPACKAGE", this.txtSrvPkg.getValue());
	    serviceConf.put("QUOTANAME", this.txtQuota.getValue());
